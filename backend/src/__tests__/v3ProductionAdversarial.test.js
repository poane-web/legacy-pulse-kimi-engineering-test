 ...
 * These tests are intentionally attacker-driven. A failure is a security
 * finding until the root cause is fixed; do not weaken assertions to obtain CI green.
 */
require('./testSetup');
...
const app = createApp();
let sequence = 0;
const uniqueEmail = (prefix) => `${prefix}-${process.pid}-${++sequence}@example.com`;

async function registerAndLogin(email, password = 'SuperSecret9') {
  const registered = await request(app).post('/api/auth/register').send({ email, password, fullName: email.split('@')[0] });
...
async function createBeneficiary(ownerToken, email) {
  ...
  const created = await request(app).post('/api/beneficiaries').set('Authorization', `Bearer ${ownerToken}`).send({
    fullName: 'Milestone Child', email, relationship: 'Child',
  });
  expect(created.status).toBe(201);
  const rawToken = new URL(`http://localhost${created.body.inviteLink}`).searchParams.get('token');
  const claimed = await request(app).post('/api/beneficiaries/claim').set('Authorization', `Bearer ${beneficiaryToken}`).send({ token: rawToken });
...
async function createTrustedContact(ownerToken, email, name) {
  ...
  const created = await request(app).post('/api/trusted-contacts').set('Authorization', `Bearer ${ownerToken}`).send({ fullName: name, email });
  expect(created.status).toBe(201);
  const rawToken = new URL(`http://localhost${created.body.inviteLink}`).searchParams.get('token');
  const claimed = await request(app).post('/api/trusted-contacts/claim').set('Authorization', `Bearer ${contactToken}`).send({ token: rawToken });
  expect(claimed.status).toBe(200);
...
async function createConfirmationMessage(ownerToken, beneficiaryId) {
  const created = await request(app).post('/api/legacy-messages').set('Authorization', `Bearer ${ownerToken}`).send({
    beneficiaryId, title: 'V3 confirmation target', body: 'Sensitive milestone payload', releaseType: 'trusted_contact_confirmation',
  });
  expect(created.status).toBe(201);
...
async function createScheduledMessage(ownerToken, beneficiaryId, releaseAt) {
  const created = await request(app).post('/api/legacy-messages').set('Authorization', `Bearer ${ownerToken}`).send({
    beneficiaryId, title: 'V3 scheduled target', body: 'Sensitive scheduled payload', releaseType: 'scheduled_date', releaseAt,
  });
  expect(created.status).toBe(201);
  return created.body.message.id;
}
...
describe('V3 production-candidate adversarial audit', () => {
  test('ATTACK BLOCKED: final trusted-contact confirmation snapshots the recipient atomically', async () => {
    ...
    const first = await request(app).post(`/api/trusted-contacts/confirm/${messageId}`).set('Authorization', `Bearer ${contactA.token}`);
    expect(first.status).toBe(200);
    expect(first.body.released).toBe(false);

    const second = await request(app).post(`/api/trusted-contacts/confirm/${messageId}`).set('Authorization', `Bearer ${contactB.token}`);
    ...
    const inbox = await request(app).get('/api/legacy-messages/inbox').set('Authorization', `Bearer ${beneficiary.token}`);
    expect(inbox.status).toBe(200);
    expect(inbox.body.messages.some((m) => m.id === messageId)).toBe(true);

    const read = await request(app).get(`/api/legacy-messages/${messageId}/read`).set('Authorization', `Bearer ${beneficiary.token}`);
    expect(read.status).toBe(200);
    expect(read.body.message.body).toBe('Sensitive milestone payload');
  });

  test('ATTACK BLOCKED: beneficiary identity mutation after first confirmation fails closed', async () => {
  ...
  test('ATTACK BLOCKED: duplicate scheduler execution cannot create duplicate recipient notifications', async () => {
    ...
    const notifications = db.prepare("SELECT COUNT(*) AS c FROM notifications WHERE type = 'message_released' AND user_id = ?").get(row.released_recipient_user_id);
  ...
  test('ATTACK BLOCKED: disabled owner JWT cannot mutate release configuration', async () => {
    ...
    const edited = await request(app).put(`/api/legacy-messages/${messageId}`).set('Authorization', `Bearer ${ownerToken}`).send({ title: 'ATTACKED' });
