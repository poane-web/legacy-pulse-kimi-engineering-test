...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    ).run(req.user.id, title, description ? encryptField(description) : null, eventDate, category || null);
  ...
  asyncHandler(async (req, res) => {
    ...
    db.prepare(
      `UPDATE life_events SET
       ...
       WHERE id = ?`
    ).run(title ?? null, eventDate ?? null, description !== undefined ? encryptField(description) : null, category ?? null, req.params.id);
