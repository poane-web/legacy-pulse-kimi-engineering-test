...
CREATE TABLE IF NOT EXISTS release_confirmations (id INTEGER PRIMARY KEY AUTOINCREMENT, owner_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, trusted_contact_id INTEGER NOT NULL REFERENCES trusted_contacts(id) ON DELETE CASCADE, confirmed_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')), UNIQUE(owner_id,trusted_contact_id));
