...
CREATE TABLE IF NOT EXISTS beneficiaries (id INTEGER PRIMARY KEY AUTOINCREMENT, owner_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, full_name TEXT NOT NULL, email TEXT NOT NULL, relationship TEXT, linked_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL, invite_token_hash TEXT, invite_status TEXT NOT NULL DEFAULT 'pending' CHECK (invite_status IN ('pending','claimed','revoked')), created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')));
...
CREATE TABLE IF NOT EXISTS trusted_contacts (id INTEGER PRIMARY KEY AUTOINCREMENT, owner_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, full_name TEXT NOT NULL, email TEXT NOT NULL, linked_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL, invite_token_hash TEXT, status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','active','revoked')), created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')));
...
CREATE TABLE IF NOT EXISTS legacy_messages (id INTEGER PRIMARY KEY AUTOINCREMENT, owner_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, beneficiary_id INTEGER NOT NULL REFERENCES beneficiaries(id) ON DELETE CASCADE, title TEXT NOT NULL, body_encrypted TEXT NOT NULL, release_type TEXT NOT NULL CHECK (release_type IN ('scheduled_date','trusted_contact_confirmation','immediate')), release_at TEXT, required_confirmations INTEGER NOT NULL DEFAULT 2, status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','released','revoked')), released_at TEXT, created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')));
CREATE INDEX IF NOT EXISTS idx_legacy_messages_owner ON legacy_messages(owner_id); CREATE INDEX IF NOT EXISTS idx_legacy_messages_beneficiary ON legacy_messages(beneficiary_id); CREATE INDEX IF NOT EXISTS idx_legacy_messages_release_scan ON legacy_messages(status,release_type,release_at);
CREATE TABLE IF NOT EXISTS release_confirmations (id INTEGER PRIMARY KEY AUTOINCREMENT, owner_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, legacy_message_id INTEGER REFERENCES legacy_messages(id) ON DELETE CASCADE, trusted_contact_id INTEGER NOT NULL REFERENCES trusted_contacts(id) ON DELETE CASCADE, confirmed_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')));
CREATE UNIQUE INDEX IF NOT EXISTS ux_release_confirmations_message_contact ON release_confirmations(legacy_message_id, trusted_contact_id) WHERE legacy_message_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_release_confirmations_message ON release_confirmations(legacy_message_id);
